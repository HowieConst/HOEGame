...
public static partial class ReferencePool
{
	...
	private static List<IReference> cacheReleaseList = new List<IReference>();


	public static void Update()
    {
		if (cacheReleaseList.Count < 1)
		{
			return;
		}
		for (int i = cacheReleaseList.Count - 1; i >= 0; i--)
		{
			OnRelease(cacheReleaseList[i]);
			cacheReleaseList.RemoveAt(i);
		}
	}
	...
	public static void Release(IReference reference)
	{
		...
		cacheReleaseList.Add(reference); 
	...
	private static void OnRelease(IReference reference)
    {
		Type referenceType = reference.GetType();
		InternalCheckReferenceType(referenceType);
		GetReferenceCollection(referenceType).Release(reference);
